...
ui_text_window :: proc(window_loc: Dimensions, title: string, border_size: i32, minimized: ^bool) {
    ...
    // window body
    rl.DrawRectangle(render_x, render_y, window_loc.width, window_loc.height, rl.Color{63, 95, 111, 255})
    ...
    title_font_size := title_height - border_size * 2
    ...
    if (rl.GuiLabelButton(rl.Rectangle{f32(render_x), f32(render_y), f32(window_loc.width), f32(title_height)}, title_text)) {
        minimized^ = !minimized^
    }

    // window contents
    render_x = window_loc.x + border_size
    render_y = render_y + title_height
    content_loc := Dimensions{render_x, render_y, window_loc.width - border_size * 2, window_loc.height - title_height - border_size}
    rl.DrawRectangle(
