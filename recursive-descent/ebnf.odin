...
get_symbol :: proc() {
	...
	case '=':
		parser.sym = .Equal
	case '(':
		parser.sym = .Left_Paren
	case ')':
		parser.sym = .Right_Paren
	case '[':
		parser.sym = .Left_Bracket
	case ']':
		parser.sym = .Right_Bracket
	case '{':
		parser.sym = .Left_Brace
	case '}':
		parser.sym = .Right_Brace
	case '|':
		parser.sym = .Bar
	case '.':
		parser.sym = .Period
...
parse :: proc(source: string, allocator := context.allocator) -> Grammar {
	...
	return Grammar{origin_loc = #location(), productions = productions}
}
...
tprint_factor :: proc(factor: Factor, allocator := context.allocator) -> string {
	...
	case .Literal:
		return fmt.tprintf("\"%s\"", factor.value.(string))
	case .Grouping:
		return fmt.tprintf("(%s)", tprint(factor.value.(Expression), allocator))
	case .Optional:
		return fmt.tprintf("[%s]", tprint(factor.value.(Expression), allocator))
	case .Repetition:
		return fmt.tprintf("{{%s}}", tprint(factor.value.(Expression), allocator))
